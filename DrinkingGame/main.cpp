...
struct DrinkerPool 
{
	...
	condition_variable conditionalVariable2;
};
...
bool TryToGetResources(Drinker *currentDrinker)
{
	...
	for (int i = 0; i < currentDrinker->resourcePool->totalResources; i++)
	{
		...
		{
			if (currentDrinker->resourcePool->resources[i].resourceMutex.try_lock())
			{
				if (currentDrinker->resourcePool->resources[i].type == ResourceType::Bottle)
				{
					currentDrinker->bottle = &currentDrinker->resourcePool->resources[i];
					return true;
				}
				else
				{
					currentDrinker->opener = &currentDrinker->resourcePool->resources[i];
					return true;
				}
			}
		}
...
void StartDrinker(Drinker *currentDrinker)
{
	...
	while (true)
	{
		...
		currentDrinker->drinkerPool->mutexC.lock();

		if (currentDrinker->drinkerPool->stopDrinkingFlag)
		{
			currentDrinker->drinkerPool->mutexC.unlock();
			break;
		}
		currentDrinker->drinkerPool->mutexC.unlock();
	}
...
void DrinkerThreadEntrypoint(Drinker *currentDrinker)
{
	...
	////////////////////////////////////////////////////////////////////////////////////////////////
	unique_lock<mutex> lock(currentDrinker->drinkerPool->mutexL);
	currentDrinker->drinkerPool->mutexC.lock();
	currentDrinker->drinkerPool->cNum++;
	currentDrinker->drinkerPool->conditionalVariable.notify_all();
	currentDrinker->drinkerPool->mutexC.unlock();
	currentDrinker->drinkerPool->conditionalVariable2.wait(lock, [&]()
		{return currentDrinker->drinkerPool->startingGunFlag == true; });
	lock.unlock();
...
int main(int argc, char **argv)
{
	...
	///////////////////////////////////////////////////////////////////////////////////
	unique_lock<mutex> numLck(poolOfDrinkers.drinkers[0].drinkerPool->mutexC);
	for (int i = 0; i < drinkerCount; i++)
	{
		thread(DrinkerThreadEntrypoint, &poolOfDrinkers.drinkers[i]).detach();
	}
	...
	///////////////////////////////////////////////////////////////////////////////////
	poolOfDrinkers.drinkers[0].drinkerPool->conditionalVariable.wait(numLck, [&]() { return poolOfDrinkers.drinkers[0].drinkerPool->cNum == drinkerCount; });
	numLck.unlock();
	...
	poolOfDrinkers.conditionalVariable2.notify_all();
	...
	//   safe manner.
	///////////////////////////////////////////////////////////////////////////////////
	poolOfDrinkers.drinkers[0].drinkerPool->mutexC.lock();
	poolOfDrinkers.drinkers[0].drinkerPool->stopDrinkingFlag = true;
	poolOfDrinkers.drinkers[0].drinkerPool->mutexC.unlock();

	///////////////////////////////////////////////////////////////////////////////////
	// TODO:: Wait for all drinkers to finish. You may not use a spinlock here, you
	//   must wait for changes in the pool of drinkers to avoid burning CPU cycles.
	///////////////////////////////////////////////////////////////////////////////////
	unique_lock<mutex> uniLck(poolOfDrinkers.drinkers[0].drinkerPool->mutexC);
	poolOfDrinkers.drinkers[0].drinkerPool->conditionalVariable.wait(uniLck, [&]() { return poolOfDrinkers.drinkers[0].drinkerPool->cNum == 0; });
	uniLck.unlock();
